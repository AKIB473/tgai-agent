...
async def test_admin_from_env():
    ...
    from tgai_agent.security.permissions import get_permission_level, PermissionLevel
...
async def test_banned_user_is_banned():
    from tgai_agent.storage.database import init_db
    from tgai_agent.storage.repositories.user_repo import upsert_user, ban_user
    from tgai_agent.security.permissions import get_permission_level, PermissionLevel
    await init_db()
