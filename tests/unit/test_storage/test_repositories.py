...
async def test_upsert_and_get_user():
    ...
    from tgai_agent.storage.repositories.user_repo import upsert_user, get_user
    await init_db()
...
async def test_ban_user():
    from tgai_agent.storage.database import init_db
    from tgai_agent.storage.repositories.user_repo import upsert_user, ban_user, is_banned
    await init_db()
...
async def test_list_users():
    from tgai_agent.storage.database import init_db
    from tgai_agent.storage.repositories.user_repo import upsert_user, list_users
    await init_db()
...
async def test_api_key_save_and_get():
    from tgai_agent.storage.database import init_db
    from tgai_agent.storage.repositories.user_repo import upsert_user
    from tgai_agent.storage.repositories.chat_repo import save_api_key, get_api_key
    await init_db()
...
async def test_clear_messages():
    from tgai_agent.storage.database import init_db
    from tgai_agent.storage.repositories.user_repo import upsert_user
    from tgai_agent.storage.repositories.chat_repo import append_message, get_messages, clear_messages
    await init_db()
...
async def test_task_create_and_list():
    ...
    task_id = await create_task(
        user_id=30001, name="Test Task",
        trigger_type="interval", trigger_value="3600",
        action_type="message", action_payload={"chat_id": 123, "text": "hello"},
    )
    assert task_id
...
async def test_task_get():
    ...
    task_id = await create_task(
        user_id=30002, name="Cron Task",
        trigger_type="cron", trigger_value="0 9 * * *",
        action_type="message", action_payload={"chat_id": 456, "text": "morning!"},
    )
...
async def test_task_delete():
    ...
    task_id = await create_task(
        user_id=30003, name="Delete Me",
        trigger_type="once", trigger_value="2030-01-01T00:00:00",
        action_type="message", action_payload={"chat_id": 1, "text": "bye"},
    )
...
async def test_agent_create_and_get():
    ...
    agent_id = await create_agent(
        user_id=40001, name="TestAgent", role="assistant",
        system_prompt="Be helpful.", ai_provider="openai", ai_model="gpt-4o-mini",
    )
...
async def test_agent_update_state():
    from tgai_agent.storage.database import init_db
    from tgai_agent.storage.repositories.user_repo import upsert_user
    from tgai_agent.storage.repositories.agent_repo import create_agent, get_agent, update_agent_state
    await init_db()
    await upsert_user(40002)
    agent_id = await create_agent(
        user_id=40002, name="StateAgent", role="coder",
        system_prompt="Write code.", ai_provider="openai", ai_model="gpt-4o-mini",
    )
...
async def test_agent_delete():
    ...
    agent_id = await create_agent(
        user_id=40003, name="Deletable", role="writer",
        system_prompt="Write.", ai_provider="gemini", ai_model="gemini-1.5-flash",
    )
